...
describe('PedidoService', () => {
  ...
  const apiUrl = 'http://localhost:8080/api/v1/orders'; // Base API URL from service
  ...
  describe('getVoucherUrl', () => {
    it('should call GET /orders/:id/voucher-url (not /pedidos)', () => {
      const id = 42;
      service.getVoucherUrl(id).subscribe();
      const req = httpMock.expectOne(`${apiUrl}/${id}/voucher-url`);
      expect(req.request.method).toBe('GET');
      req.flush({ data: 'https://storage.example/voucher.jpg' });
    });
  });
});
